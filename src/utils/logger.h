...
#define LOG_DEBUG(...) ::util::Logger::debug_at(__FILE__, __LINE__, __VA_ARGS__)
#else
#define LOG_DEBUG(...) \
  do {                 \
  } while (0)
...
#define LOG_INFO(...) ::util::Logger::info_at(__FILE__, __LINE__, __VA_ARGS__)
#else
#define LOG_INFO(...) \
  do {                \
  } while (0)
...
#define LOG_WARN(...) ::util::Logger::warn_at(__FILE__, __LINE__, __VA_ARGS__)
#else
#define LOG_WARN(...) \
  do {                \
  } while (0)
...
#define LOG_ERROR(...) ::util::Logger::error_at(__FILE__, __LINE__, __VA_ARGS__)
#else
#define LOG_ERROR(...) \
  do {                 \
  } while (0)
...
#define LOG_FATAL(...) ::util::Logger::fatal_at(__FILE__, __LINE__, __VA_ARGS__)
#else
#define LOG_FATAL(...) \
  do {                 \
  } while (0)
